...
cat("\nRunning modelAgeSexArea_v5.jags ...\n")
out_v5 <- jags(
  data               = jags_data,
  inits              = inits,
  parameters.to.save = params,
  model.file         = here::here(jags, "modelAgeSexArea_v5.jags"),
  n.chains           = 3,
  n.iter             = 30000,
  n.burnin           = 10000,
  n.thin             = 2,
...
print(data.frame(
  Region = region_names,
  Mean   = round(out_v5$mean$lambda, 3),
  Lower  = round(out_v5$q2.5$lambda, 3),
  Upper  = round(out_v5$q97.5$lambda, 3)
))

cat("\nPosterior mean survival p[group, age]:\n")
p_mat <- round(out_v5$mean$p, 3)
...
pdf("ppc_plot_v5.pdf", width = 6, height = 6)
plot(out_v5$sims.list$cs_obs, out_v5$sims.list$cs_new,
...
dev.off()
cat("\nPPC plot saved to ppc_plot_v5.pdf\n")
...
for (i in 1:G) {
  ...
  ey[1] <- out_v5$mean$lambda[area[i]] - sum(ey[2:S])
  theta_hat[i, ] <- ey / sum(ey)
}

pdf("obs_vs_exp_v5.pdf", width = 12, height = 6)
...
dev.off()
cat("Observed vs expected plot saved to obs_vs_exp_v5.pdf\n")

## -------------------------------------------------------
## 10.  SURVIVAL PLOT
## -------------------------------------------------------

pdf("survival_v5.pdf", width = 10, height = 5)
par(mfrow = c(1, 2))
...
for (sx in 1:2) {
  plot(NA,
    ...
    main = paste(sex_names[sx], "- v5 model")
  )
...
dev.off()
cat("Survival plot saved to survival_v5.pdf\n")

saveRDS(out_v5, "out_v5.rds")
cat("\nModel output saved to out_v5.rds\n")
cat("Done.\n")

## Full parameter table for v5
params_summary <- data.frame(
  ...
  Mean = round(c(
    out_v5$mean$b_intercept,
    out_v5$mean$b_age,
    out_v5$mean$b_region[2:4],
    out_v5$mean$b_sex
  ), 3),
  Lower = round(c(
    out_v5$q2.5$b_intercept,
    out_v5$q2.5$b_age,
    out_v5$q2.5$b_region[2:4],
    out_v5$q2.5$b_sex
  ), 3),
  Upper = round(c(
    out_v5$q97.5$b_intercept,
    out_v5$q97.5$b_age,
    out_v5$q97.5$b_region[2:4],
    out_v5$q97.5$b_sex
  ), 3)
)
print(params_summary)

out_v5$mean$b_region[1:4]
