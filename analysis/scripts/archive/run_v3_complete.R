...
##   3. Builds the y matrix for modelAgeSexArea_v3.jags
...
cat("\nRunning modelAgeSexArea_v3.jags ...\n")
out_v3 <- jags(
  data               = jags_data,
  inits              = inits,
  parameters.to.save = params,
  model.file         = here::here(jags, "modelAgeSexArea_v3.jags"),
  n.chains           = 3,
  n.iter             = 30000,
  n.burnin           = 10000,
  n.thin             = 2,
...
cat(sprintf(
  ...
  out_v3$mean$phi, out_v3$q2.5$phi, out_v3$q97.5$phi
))
...
lambda_df <- data.frame(
  Region = region_names,
  Mean   = round(out_v3$mean$lambda, 3),
  Lower  = round(out_v3$q2.5$lambda, 3),
  Upper  = round(out_v3$q97.5$lambda, 3)
)
print(lambda_df)

## Survival probability summary
cat("\nPosterior mean survival p[group, age]:\n")
p_mat <- round(out_v3$mean$p, 3)
...
pdf("ppc_plot_v3.pdf", width = 6, height = 6)
plot(out_v3$sims.list$cs_obs, out_v3$sims.list$cs_new,
...
dev.off()
cat("\nPPC plot saved to ppc_plot_v3.pdf\n")
...
pdf("survival_v3.pdf", width = 10, height = 5)
par(mfrow = c(1, 2))
cols <- c("black", "blue", "red", "darkgreen")

for (sx in 1:2) {
  plot(NA,
    xlim = c(0, 8), ylim = c(0.1, 1.0),
    xlab = "Age class", ylab = "Survival probability",
    main = paste(sex_names[sx], "- v3 model")
  )
  abline(h = 0.5, lty = 2, col = "grey70")

  for (r in 1:4) {
    g <- (r - 1) * 2 + sx
    p_mean <- out_v3$mean$p[g, ]
    p_lo <- out_v3$q2.5$p[g, ]
    p_hi <- out_v3$q97.5$p[g, ]
...
dev.off()
cat("Survival plot saved to survival_v3.pdf\n")

## Save model output
saveRDS(out_v3, "out_v3.rds")
cat("\nModel output saved to out_v3.rds\n")
cat("\nDone.\n")
