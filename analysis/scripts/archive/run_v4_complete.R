...
##   3. Builds the y matrix for modelAgeSexArea_v4.jags
...
## Pool ages >= 7 into age class 7  ("7+" terminal class)
## This reduces S from 9 to 8 and eliminates zero cells
...
##
## Stage classes S = 8  (age 0 .. 7, where 7 = "7+")
...
cat("\nCount matrix y[G, S]  (rows=groups, cols=age classes 0..7):\n")
rownames(y) <- paste0(
...
inits <- function() {
  list(
    ...
    b_age       = c(NA, rep(0, S - 1)), # NA = age[1] fixed at 0
    log_phi     = 3,
...
cat("\nRunning modelAgeSexArea_v4.jags ...\n")
out_v4 <- jags(
  data               = jags_data,
  inits              = inits,
  parameters.to.save = params,
  model.file         = here::here(jags, "modelAgeSexArea_v4.jags"),
  n.chains           = 3,
  n.iter             = 30000,
  n.burnin           = 10000,
  n.thin             = 2,
...
cat(sprintf(
  ...
  out_v4$mean$phi, out_v4$q2.5$phi, out_v4$q97.5$phi
))
...
lambda_df <- data.frame(
  Region = region_names,
  Mean   = round(out_v4$mean$lambda, 3),
  Lower  = round(out_v4$q2.5$lambda, 3),
  Upper  = round(out_v4$q97.5$lambda, 3)
)
print(lambda_df)

## Survival probability summary
cat("\nPosterior mean survival p[group, age]:\n")
p_mat <- round(out_v4$mean$p, 3)
rownames(p_mat) <- rownames(y)
colnames(p_mat) <- paste0("age", 0:7)
print(p_mat)
...
pdf("ppc_plot_v4.pdf", width = 6, height = 6)
plot(out_v4$sims.list$cs_obs, out_v4$sims.list$cs_new,
...
dev.off()
cat("\nPPC plot saved to ppc_plot_v4.pdf\n")
...
pdf("survival_v4.pdf", width = 10, height = 5)
par(mfrow = c(1, 2))
...
for (sx in 1:2) {
  plot(NA,
    ...
    main = paste(sex_names[sx], "- v4 model")
  )
  abline(h = 0.5, lty = 2, col = "grey70")

  for (r in 1:4) {
    g <- (r - 1) * 2 + sx
    p_mean <- out_v4$mean$p[g, ]
    p_lo <- out_v4$q2.5$p[g, ]
    p_hi <- out_v4$q97.5$p[g, ]
    lines(0:7, p_mean, col = cols[r], lwd = 2)
    points(0:7, p_mean, col = cols[r], pch = r, cex = 1.2)
    segments(0:7, p_lo, 0:7, p_hi, col = cols[r], lwd = 0.8)
  }
  if (sx == 1) {
    legend("topright",
      legend = c("N", "NC", "SC", "S"),
...
dev.off()
cat("Survival plot saved to survival_v4.pdf\n")

## Save model output
saveRDS(out_v4, "out_v4.rds")
cat("\nModel output saved to out_v4.rds\n")
cat("\nDone.\n")
