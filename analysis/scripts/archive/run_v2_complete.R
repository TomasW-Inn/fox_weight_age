...
##   2 = N   (Lappland, Norrbotten, Västerbotten)q()
...
out_v2 <- jags(
  ...
  model.file         = here(jags, "modelAgeSexArea_v2.jags"),
  n.chains           = 3,
  n.iter             = 30000,
  n.burnin           = 10000,
  n.thin             = 2,
...
str(out_v2$sims.list)
# Which groups contribute most to cs_obs?
g_obs_mean <- apply(out_v2$sims.list$cs_obs, 2, mean)
# (if monitored per group)
